...
@with_kw mutable struct LazPoint{Trans}
    X::Int32 = Int32(0)
    Y::Int32 = Int32(0)
    Z::Int32 = Int32(0)
...
transform(::LazPoint{Trans}) where Trans = Trans

function Base.propertynames(point::LazPoint)
    (fieldnames(typeof(point))..., :x, :y, :z, :xyz)
end

function Base.getproperty(point::LazPoint, sym::Symbol)
    if sym === :xyz
        transform(point)(point.X, point.Y, point.Z)
    elseif sym === :x
        transform(point)(point.X, point.Y, point.Z)[1]
    elseif sym === :y
        transform(point)(point.X, point.Y, point.Z)[2]
    elseif sym === :z
        transform(point)(point.X, point.Y, point.Z)[3]
    else
        return getfield(point, sym)
    end
end
