...
function laszip_get_version(version_major, version_minor, version_revision, version_build)
    ccall((:laszip_get_version, laszip), Int16, (Ptr{UInt8}, Ptr{UInt8}, Ptr{UInt16}, Ptr{UInt32}), version_major, version_minor, version_revision, version_build)
end

function laszip_create(pointer::Ref{Ptr{Void}})
    ccall((:laszip_create, laszip), Int16, (Ptr{Ptr{Void}},), pointer)
end

function laszip_get_error(pointer::Ptr{Void}, error)
    ccall((:laszip_get_error, laszip), Int16, (Ptr{Void}, Ptr{Ptr{laszip_CHAR}}), pointer, error)
end

function laszip_get_warning(pointer::Ptr{Void}, warning)
    ccall((:laszip_get_warning, laszip), Int16, (Ptr{Void}, Ptr{Ptr{laszip_CHAR}}), pointer, warning)
end

function laszip_clean(pointer::Ptr{Void})
    ccall((:laszip_clean, laszip), Int16, (Ptr{Void},), pointer)
end

function laszip_destroy(pointer::Ptr{Void})
    ccall((:laszip_destroy, laszip), Int16, (Ptr{Void},), pointer)
end

function laszip_get_header_pointer(pointer::Ptr{Void}, header_pointer)
    ccall((:laszip_get_header_pointer, laszip), Int16, (Ptr{Void}, Ref{Ptr{Header}}), pointer, header_pointer)
end

function laszip_get_point_pointer(pointer::Ptr{Void}, point_pointer)
    ccall((:laszip_get_point_pointer, laszip), Int16, (Ptr{Void}, Ref{Ptr{Point}}), pointer, point_pointer)
end

function laszip_get_point_count(pointer::Ptr{Void}, count)
    ccall((:laszip_get_point_count, laszip), Int16, (Ptr{Void}, Ptr{Int64}), pointer, count)
end

function laszip_set_header(pointer::Ptr{Void}, header)
    ccall((:laszip_set_header, laszip), Int16, (Ptr{Void}, Ptr{Void}), pointer, header)
end

function laszip_set_point_type_and_size(pointer::Ptr{Void}, point_type::UInt8, point_size::UInt16)
    ccall((:laszip_set_point_type_and_size, laszip), Int16, (Ptr{Void}, UInt8, UInt16), pointer, point_type, point_size)
end

function laszip_check_for_integer_overflow(pointer::Ptr{Void})
    ccall((:laszip_check_for_integer_overflow, laszip), Int16, (Ptr{Void},), pointer)
end

function laszip_auto_offset(pointer::Ptr{Void})
    ccall((:laszip_auto_offset, laszip), Int16, (Ptr{Void},), pointer)
end

function laszip_set_point(pointer::Ptr{Void}, point)
    ccall((:laszip_set_point, laszip), Int16, (Ptr{Void}, Ptr{Void}), pointer, point)
end

function laszip_set_coordinates(pointer::Ptr{Void}, coordinates)
    ccall((:laszip_set_coordinates, laszip), Int16, (Ptr{Void}, Ptr{Float64}), pointer, coordinates)
end

function laszip_get_coordinates(pointer::Ptr{Void}, coordinates)
    ccall((:laszip_get_coordinates, laszip), Int16, (Ptr{Void}, Ptr{Float64}), pointer, coordinates)
end

function laszip_set_geokeys(pointer::Ptr{Void}, number::UInt32, key_entries)
    ccall((:laszip_set_geokeys, laszip), Int16, (Ptr{Void}, UInt32, Ptr{Void}), pointer, number, key_entries)
end

function laszip_set_geodouble_params(pointer::Ptr{Void}, number::UInt32, geodouble_params)
    ccall((:laszip_set_geodouble_params, laszip), Int16, (Ptr{Void}, UInt32, Ptr{Float64}), pointer, number, geodouble_params)
end

function laszip_set_geoascii_params(pointer::Ptr{Void}, number::UInt32, geoascii_params)
    ccall((:laszip_set_geoascii_params, laszip), Int16, (Ptr{Void}, UInt32, Ptr{laszip_CHAR}), pointer, number, geoascii_params)
end

function laszip_add_attribute(pointer::Ptr{Void}, _type::UInt32, name, description, scale::Float64, offset::Float64)
    ccall((:laszip_add_attribute, laszip), Int16, (Ptr{Void}, UInt32, Ptr{laszip_CHAR}, Ptr{laszip_CHAR}, Float64, Float64), pointer, _type, name, description, scale, offset)
end

function laszip_add_vlr(pointer::Ptr{Void}, user_id, record_id::UInt16, record_length_after_header::UInt16, description, data)
    ccall((:laszip_add_vlr, laszip), Int16, (Ptr{Void}, Ptr{laszip_CHAR}, UInt16, UInt16, Ptr{laszip_CHAR}, Ptr{UInt8}), pointer, user_id, record_id, record_length_after_header, description, data)
end

function laszip_remove_vlr(pointer::Ptr{Void}, user_id, record_id::UInt16)
    ccall((:laszip_remove_vlr, laszip), Int16, (Ptr{Void}, Ptr{laszip_CHAR}, UInt16), pointer, user_id, record_id)
end

function laszip_create_spatial_index(pointer::Ptr{Void}, create::laszip_BOOL, append::laszip_BOOL)
    ccall((:laszip_create_spatial_index, laszip), Int16, (Ptr{Void}, laszip_BOOL, laszip_BOOL), pointer, create, append)
end

function laszip_preserve_generating_software(pointer::Ptr{Void}, preserve::laszip_BOOL)
    ccall((:laszip_preserve_generating_software, laszip), Int16, (Ptr{Void}, laszip_BOOL), pointer, preserve)
end

function laszip_request_native_extension(pointer::Ptr{Void}, request::laszip_BOOL)
    ccall((:laszip_request_native_extension, laszip), Int16, (Ptr{Void}, laszip_BOOL), pointer, request)
end

function laszip_request_compatibility_mode(pointer::Ptr{Void}, request::laszip_BOOL)
    ccall((:laszip_request_compatibility_mode, laszip), Int16, (Ptr{Void}, laszip_BOOL), pointer, request)
end

function laszip_set_chunk_size(pointer::Ptr{Void}, chunk_size::UInt32)
    ccall((:laszip_set_chunk_size, laszip), Int16, (Ptr{Void}, UInt32), pointer, chunk_size)
end

function laszip_open_writer(pointer::Ptr{Void}, file_name, compress::laszip_BOOL)
    ccall((:laszip_open_writer, laszip), Int16, (Ptr{Void}, Ptr{laszip_CHAR}, laszip_BOOL), pointer, file_name, compress)
end

function laszip_write_point(pointer::Ptr{Void})
    ccall((:laszip_write_point, laszip), Int16, (Ptr{Void},), pointer)
end

function laszip_write_indexed_point(pointer::Ptr{Void})
    ccall((:laszip_write_indexed_point, laszip), Int16, (Ptr{Void},), pointer)
end

function laszip_update_inventory(pointer::Ptr{Void})
    ccall((:laszip_update_inventory, laszip), Int16, (Ptr{Void},), pointer)
end

function laszip_close_writer(pointer::Ptr{Void})
    ccall((:laszip_close_writer, laszip), Int16, (Ptr{Void},), pointer)
end

function laszip_exploit_spatial_index(pointer::Ptr{Void}, exploit::laszip_BOOL)
    ccall((:laszip_exploit_spatial_index, laszip), Int16, (Ptr{Void}, laszip_BOOL), pointer, exploit)
end

function laszip_decompress_selective(pointer::Ptr{Void}, decompress_selective::UInt32)
    ccall((:laszip_decompress_selective, laszip), Int16, (Ptr{Void}, UInt32), pointer, decompress_selective)
end

function laszip_open_reader(pointer::Ptr{Void}, file_name, is_compressed)
    ccall((:laszip_open_reader, laszip), Int16, (Ptr{Void}, Ptr{laszip_CHAR}, Ptr{laszip_BOOL}), pointer, file_name, is_compressed)
end

function laszip_has_spatial_index(pointer::Ptr{Void}, is_indexed, is_appended)
    ccall((:laszip_has_spatial_index, laszip), Int16, (Ptr{Void}, Ptr{laszip_BOOL}, Ptr{laszip_BOOL}), pointer, is_indexed, is_appended)
end

function laszip_inside_rectangle(pointer::Ptr{Void}, min_x::Float64, min_y::Float64, max_x::Float64, max_y::Float64, is_empty)
    ccall((:laszip_inside_rectangle, laszip), Int16, (Ptr{Void}, Float64, Float64, Float64, Float64, Ptr{laszip_BOOL}), pointer, min_x, min_y, max_x, max_y, is_empty)
end

function laszip_seek_point(pointer::Ptr{Void}, index::Int64)
    ccall((:laszip_seek_point, laszip), Int16, (Ptr{Void}, Int64), pointer, index)
end

function laszip_read_point(pointer::Ptr{Void})
    ccall((:laszip_read_point, laszip), Int16, (Ptr{Void},), pointer)
end

function laszip_read_inside_point(pointer::Ptr{Void}, is_done)
    ccall((:laszip_read_inside_point, laszip), Int16, (Ptr{Void}, Ptr{laszip_BOOL}), pointer, is_done)
end

function laszip_close_reader(pointer::Ptr{Void})
    ccall((:laszip_close_reader, laszip), Int16, (Ptr{Void},), pointer)
end

function laszip_load_dll()
    # cannot find these on windows
    @static if !is_windows()
        ccall((:laszip_load_dll, laszip), Int16, ())
    end
end
...
function laszip_unload_dll()
    # cannot find these on windows
    @static if !is_windows()
        ccall((:laszip_unload_dll, laszip), Int16, ())
    end
end
